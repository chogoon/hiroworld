...
public class NewCollapsingTextHelper {
    ...
    public NewCollapsingTextHelper(View view) {
        mView = view;
    ...
    public void draw(Canvas canvas) {
        ...
        if (mTextToDraw != null && mDrawTitle) {
            ...
            if (drawTexture) {
                ...
                int width = (int)x + 220;
                RectF topRectF = new RectF(x, y + ascent, width, y + ascent + 220);
                canvas.drawRect(topRectF, circlePaint);

                RectF bounds = new RectF(topRectF);
                bounds.right = mTextPaint.measureText(point, 0, point.length());
// measure text height
                bounds.bottom = mTextPaint.descent() - mTextPaint.ascent();

                bounds.left += (topRectF.width() - bounds.right) / 2.0f;
                bounds.top += (topRectF.height() - bounds.bottom) / 2.0f;
                canvas.drawText(point,  bounds.left, bounds.top - mTextPaint.ascent(), mTextPaint);
                canvas.drawText(mTextToDraw, 0, mTextToDraw.length(), x + bounds.right + 20 , bounds.top - mTextPaint.ascent(), mTextPaint);
            }
        }
